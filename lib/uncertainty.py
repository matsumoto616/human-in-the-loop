...
class Uncertainty:
    ...
    def get_uncertainty(self):
        ...
        if self.sample_type == "attention":
            ...
            #     input_tensor=inputs_l, targets=None)

            for (inputs, labels, _, _, _, _) in self.unlabeled_loader:
                attention_map_unl = self.cam(
                    input_tensor=inputs, targets=None)
                uncertainty = torch.cat(
                    (uncertainty, self.attention_distribution_based(attention_map_unl)), 0)
        else:
            with torch.no_grad():
                for (inputs, labels, _, _, _, _) in self.unlabeled_loader:
    ...
    def random_based(self, prob_dist):
        random_indices = torch.randperm(prob_dist.shape[0])
    ...
    def entropy_based(self, prob_dist):
        ...
        normalized_entropy = raw_entropy / torch.log2(torch.from_numpy(
            np.full(prob_dist.shape[0], prob_dist.shape[1]).astype(np.float32)).clone())
