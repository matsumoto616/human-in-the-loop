...
def load_network():
    ...
    active_learning = Uncertainty(network, added_number, train_set, uncertainty_train_set, test_set, val_set,
...
def show_next():
    ...
    inputs, labels, _, _, _, _ = next(loader_iter)
    if show_iter == 0:
    ...
    print(inputs)
    print(labels)
    grad_cam = cam(input_tensor=inputs, targets=labels,)
